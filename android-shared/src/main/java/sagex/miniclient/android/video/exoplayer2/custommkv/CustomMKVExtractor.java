...
// cncb - Copied this file from ExoPlayer source and the 4 others in this directory just to support DTS-HD
// pass-through by commenting out a few lines (2081-82). Standard DTS will fail to play with this extractor.

package sagex.miniclient.android.video.exoplayer2.custommkv;
...
public class CustomMKVExtractor implements Extractor {

  /** Factory for {@link CustomMKVExtractor} instances. */
  public static final ExtractorsFactory FACTORY = () -> new Extractor[] {new CustomMKVExtractor()};
  ...
  private static final String TAG = "CustomMKVExtractor";
  ...
  public CustomMKVExtractor() {
    this(0);
  }

  public CustomMKVExtractor(@Flags int flags) {
    this(new DefaultEbmlReader(), flags);
  }

  /* package */ CustomMKVExtractor(EbmlReader reader, @Flags int flags) {
    this.reader = reader;
  ...
  /** Passes events through to the outer {@link CustomMKVExtractor}. */
  private final class InnerEbmlProcessor implements EbmlProcessor {

    @Override
    @ElementType
    public int getElementType(int id) {
      return CustomMKVExtractor.this.getElementType(id);
    }

    @Override
    public boolean isLevel1Element(int id) {
      return CustomMKVExtractor.this.isLevel1Element(id);
    }

    @Override
    public void startMasterElement(int id, long contentPosition, long contentSize)
        throws ParserException {
      CustomMKVExtractor.this.startMasterElement(id, contentPosition, contentSize);
    }

    @Override
    public void endMasterElement(int id) throws ParserException {
      CustomMKVExtractor.this.endMasterElement(id);
    }

    @Override
    public void integerElement(int id, long value) throws ParserException {
      CustomMKVExtractor.this.integerElement(id, value);
    }

    @Override
    public void floatElement(int id, double value) throws ParserException {
      CustomMKVExtractor.this.floatElement(id, value);
    }

    @Override
    public void stringElement(int id, String value) throws ParserException {
      CustomMKVExtractor.this.stringElement(id, value);
    }

    @Override
    public void binaryElement(int id, int contentsSize, ExtractorInput input) throws IOException {
      CustomMKVExtractor.this.binaryElement(id, contentsSize, input);
    }
  }
  ...
  private static final class Track {
    ...
    public void initializeOutput(ExtractorOutput output, int trackId) throws ParserException {
      ...
      switch (codecId) {
        ...
        case CODEC_ID_DTS:
        case CODEC_ID_DTS_EXPRESS:
// cncb - for DTS-HD pass-through. Standard DTS will fail to play.
...
//                    break;
     ...
     *
     * <p>It is unfortunately not possible to mark {@link CustomMKVExtractor#tracks} as only
